...
Ресторая "Ятъ". АлклгольFREE. Cильно маленький (4 столика, 8 работников).
			...
			for меню in select * from меню loop
				...
				if меню.наименование = 'Основное меню' then
					...
					for тип_блюда in select * from типы_блюд where меню_id = меню.id  loop
							execute 'insert into заказанные_блюда (заказ_id , блюдо_id) values ($1::int, (select рб.id from блюда рб join типы_блюд ртб on рб.тип_id = ртб.id where ртб.id  =  $2::int order by random() limit 1))'
								using заказ_id, тип_блюда.id;
					end loop;
				else
					-- Десерт и напиток
					execute 'insert into заказанные_блюда (заказ_id , блюдо_id) values ($1::int,(select рб.id from блюда рб join типы_блюд ртб on рб.тип_id = ртб.id join меню рм on рм.id = ртб.меню_id where рм.id  = $2::int order by random() limit 1))'
